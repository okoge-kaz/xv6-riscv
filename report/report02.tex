...
また、\code{>>} (リダイレクション)に関しては、\code{make qemu}後に、実際に\code{echo hello >> test.txt}を実行し、ファイルの末尾に文字列が追加されていることを確認した。
...
方針としては、typeがFD\_INODEの場合に、\code{f->off}を変更することで、lseekを実現することにした。
